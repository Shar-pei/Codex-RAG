...
#         f"(attempt {retry_state.attempt_number} of 3). Error: {retry_state.outcome.exception()}"
#     ),
#
# )
async def openai_complete_if_cache(
    ...
    **kwargs: Any,
    ...
    kwargs = prepare_openai_kwargs(**kwargs)
    ...
    for attempt in range(1, 4):  # 最多重试3次
        try:
            ...
            last_exception = e
            if attempt < 3:
                wait_time = 4 * (2 ** (attempt - 1))  # 4, 8, 16秒
                logger.warning(
                    f"OpenAI API request failed (attempt {attempt}/3), "
...
def prepare_openai_kwargs(**kwargs):
    """
    准备 OpenAI API 调用参数，处理 enable_thinking 等敏感参数
    """
    ...
    for key, default_value in defaults.items():
        if key not in prepared_kwargs:
            prepared_kwargs[key] = default_value

    # 特殊处理：确保非流式调用时 enable_thinking 为 False
    if not prepared_kwargs.get('stream', False):
        prepared_kwargs['extra_body']['enable_thinking'] = False

    # 记录参数调整（用于调试）
    # if 'enable_thinking' in kwargs and kwargs['enable_thinking'] != prepared_kwargs['enable_thinking']:
    #     logger.debug(
    #         f"Adjusted enable_thinking from {kwargs['enable_thinking']} to {prepared_kwargs['enable_thinking']}")
