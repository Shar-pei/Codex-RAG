...
def display_splash_screen(args: argparse.Namespace) -> None:
    ...
    if stdout_encoding and stdout_encoding != "utf-8":
        protocol = "https" if args.ssl else "http"
        base_url = (
            f"{protocol}://localhost:{args.port}"
            if args.host == "0.0.0.0"
            else f"{protocol}://{args.host}:{args.port}"
        )
        ...
        ASCIIColors.white("  - LLM Binding: ", end="")
        ASCIIColors.yellow(f"{args.llm_binding}")
        ASCIIColors.white("  - LLM Model: ", end="")
        ASCIIColors.yellow(f"{args.llm_model}")
        ASCIIColors.white("  - Embedding Binding: ", end="")
        ASCIIColors.yellow(f"{args.embedding_binding}")
        ASCIIColors.white("  - Embedding Model: ", end="")
        ASCIIColors.yellow(f"{args.embedding_model}")
        ASCIIColors.white("  - WebUI: ", end="")
        ASCIIColors.yellow(base_url)
        ASCIIColors.white("  - API Docs: ", end="")
