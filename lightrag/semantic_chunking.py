...
def semantic_chunking_by_token_size(
    ...
    """
    语义分块函数，兼容原始chunking_by_token_size接口
    为了保持与现有测试的兼容性，当需要按字符分割时，使用原始逻辑；
    否则使用语义分块逻辑。
    """
    from ..operate import ChunkTokenLimitExceededError  # 导入异常类

    # 如果指定了按字符分割，使用原始逻辑以保持兼容性
